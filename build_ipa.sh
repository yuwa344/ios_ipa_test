...
echo ">>> 归档 (archive) ..."
ARCHIVE_ARGS=(
  -project "JR_CompanyWork.xcodeproj"
  -scheme "JR_CompanyWork"
  -configuration "Release"
  -destination "generic/platform=iOS"
  -archivePath "build/JR_CompanyWork.xcarchive"
)
...
if [ "$UNSIGNED" = "1" ]; then
  echo ">>> 打包 IPA(未签名) ..."
  APP_PATH="build/JR_CompanyWork.xcarchive/Products/Applications/JR_CompanyWork.app"
  if [ ! -d "$APP_PATH" ]; then
  ...
  cd build
  zip -qry JR_CompanyWork-unsigned.ipa Payload/
  cd ..
  rm -rf build/Payload
  echo ">>> 完成:build/JR_CompanyWork-unsigned.ipa"
  ls -lh build/JR_CompanyWork-unsigned.ipa
  exit 0
fi
...
echo ">>> 完成:build/JR_CompanyWork.ipa"
ls -lh build/JR_CompanyWork.ipa
